...
public final class MigrationHelper {
     ...
     *  value: dbType
     */
    private static final Map<Class, String> propertyToDbType = new ArrayMap<Class, String>(10);
    ...
    private static void generateTempTables(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        for (int i = 0; i < daoClasses.length; i++) {
            ...
            try
            {
               ...
               // Log.i(TAG, tableName+" moveToNext="+c.moveToNext());
                if (!c.moveToNext()) {
                    if(c!=null) {
                        c.close();
                    }
            ...
            try {
                ...
                for (int j = 0; j < daoConfig.properties.length; j++) {
                    ...
                    if (columns.contains(columnName)) {
                        ...
                        try {
                            Object value = new ConvertUtilsBean().convert(0,daoConfig.properties[j].type);
                            propertiesQuery.add(value+" as " + columnName);
                            properties.add(columnName);
                        } catch (Exception e) {
                ...
                if (properties.size() > 0) {
                    ...
                    final String columnSQLL = TextUtils.join(",", propertiesQuery);

                    StringBuilder insertTableStringBuilder = new StringBuilder();
                    insertTableStringBuilder.append("INSERT INTO ").append(tableName).append(" (");
                    insertTableStringBuilder.append(columnSQL);
                    insertTableStringBuilder.append(") SELECT ");
                    insertTableStringBuilder.append(columnSQLL);
