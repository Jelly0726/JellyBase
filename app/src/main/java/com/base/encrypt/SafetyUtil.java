...
//		return map;
        Log.i("SafetyUtil", "签名加密前:" + stringBuffer.toString());
        String sign = encryptRSA(context.getApplicationContext(), stringBuffer.toString(), publicKey);
        Log.i("SafetyUtil", "签名加密后:" + sign);
        return sign;
        //return MD5(stringBuffer.toString()).toUpperCase();
    }
...
//		return map;
        Log.i("SafetyUtil", "签名加密前:" + stringBuffer.toString());
        String sign = encodeByRSAPriKey(context.getApplicationContext(), stringBuffer.toString(), privateKey);
        Log.i("SafetyUtil", "签名加密后:" + sign);
        return sign;
        //return MD5(stringBuffer.toString()).toUpperCase();
    }
...
//		return map;
        ...
        String sign;
        if (isBase64(source))
            sign = jni.decryptByAESEncrypt(context.getApplicationContext(),
                    Base64.decode(source, Base64.NO_WRAP), key.getBytes());
        else
            sign = jni.decryptByAESEncrypt(context.getApplicationContext(), source.getBytes(), key.getBytes());
        Log.i("SafetyUtil", "解密后:" + sign);
        return sign;
        //return MD5(stringBuffer.toString()).toUpperCase();
    }
...
//		return map;
        ...
        String sign;
        if (isBase64(source)) {
            sign = jni.decryptByAESEncrypt(context.getApplicationContext(), Base64.decode(source, Base64.NO_WRAP), key.getBytes());
        }else
            sign = jni.decryptAESCipher(context.getApplicationContext(), source.getBytes(), key.getBytes());
        Log.i("SafetyUtil", "解密后:" + sign);
        return sign;
        //return MD5(stringBuffer.toString()).toUpperCase();
    }
    ...
    public String decode(Context context, @NonNull String source, int type) {
        ...
        switch (type) {
            case AES:
                if (isBase64(source)) {
                    sign = jni.decodeByAESEncrypt(context.getApplicationContext(),Base64.decode(source, Base64.NO_WRAP));
    ...
    private String sign(Context context, @NonNull String source, int type) {
        ...
        switch (type) {
            case MD5:
                ...
                break;
            case HMAC_SHA1:
                sign = jni.encodeByHmacSHA1(context, source.getBytes());
                break;
            case SHA1:
