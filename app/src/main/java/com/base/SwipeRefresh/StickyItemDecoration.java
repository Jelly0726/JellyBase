...
/**
 * 分组悬浮对应的ItemDecoration
 */
public class StickyItemDecoration extends DefaultItemDecoration implements IStickyDecoration {

    private Rect mPinnedHeaderRect     = null;
    private int  mPinnedHeaderPosition = -1;

    public StickyItemDecoration(int color) {
        super(color);
    }

    public StickyItemDecoration(int color, int dividerWidth, int dividerHeight, int... excludeViewType) {
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (parent.getAdapter() instanceof StickyAdapter && parent.getChildCount() > 0) {
            ...
            if (pinnedHeaderPosition != -1) {
                ...
                if (layoutParams == null) {
                    throw new NullPointerException("StickyItemDecoration");
                }
     ...
     * @return -1：未找到 >=0 找到位置
     */
    private int getPinnedHeaderViewPosition(int adapterFirstVisible, StickyAdapter adapter) {
