 ...
 * view 添加阴影
 * 使用：
 *        ShadowDrawableUtil.setShadowDrawable(round_tv, ShadowDrawableUtil.TypeEnum.LEFT);
 *        ShadowDrawableUtil.setShadowDrawableTop(round_part_tv,"#A6000000");
 *        ShadowDrawableUtil.setShadowDrawableCircle(circle_tv);
 */
public class ShadowDrawableUtil extends Drawable {
    ...
    private ShadowDrawableUtil(int shape, TypeEnum type, int[] bgColor, int shapeRadius, int shadowColor, int shadowRadius) {
        this.mShape = shape;
        this.type = type;
    ...
    private static int dp2px(float dpVal) {
        return (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dpVal, BaseApplication.getInstance().getResources().getDisplayMetrics());
    }
}