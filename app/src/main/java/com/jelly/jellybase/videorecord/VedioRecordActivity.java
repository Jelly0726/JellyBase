...
public class VedioRecordActivity extends AppCompatActivity implements VideoRecordSurface.OnRecordListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.vedio_record_activity);
...
//                        这里因为启动录制的时候需要先初始化并不是马上开始录制所以进度条不在这里启动在回调开始时启动
//                        mProgressView.startProgress(videoRecordSurface.mRecordMaxTime);
                        break;
                    }
                    case MotionEvent.ACTION_MOVE: {
                        ...
                        if (moveY > 0 && moveX > 0 && moveX <= rect.right && moveY <= rect.bottom) {
                            ...
                            if (!"移开取消".equals(tvTips.getText().toString().trim())) {
                                ...
                                tvTips.setTextColor(Color.parseColor("#FF00FF29"));
                                tvTips.setText("移开取消");
                            }
...
//                        tvTips.setVisibility(View.INVISIBLE);
    ...
    private void initView() {
        ...
        mProgressView = (CircularProgressView) findViewById(R.id.mProgressView);
    ...
    public void onRecordProgress(int progress) {
        if (progress==0){//当progress为0时表示正式开始录制，启动进度条
            mProgressView.startProgress(videoRecordSurface.mRecordMaxTime);
        }
        iTime = progress;
        btnStart.setText(MyDate.secToTimeEN(videoRecordSurface.mRecordMaxTime-iTime));
    }
