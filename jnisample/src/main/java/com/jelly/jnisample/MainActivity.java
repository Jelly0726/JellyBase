    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_java_encode:
                result = "hmacSHA1签名编码->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.HMAC_SHA1);
                break;
            case R.id.btn_native_encode:
                result = "SHA1签名->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.SHA1);
                break;
            case R.id.btn_java_decode:
                result = "SHA224签名->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.SHA224);
                break;
            case R.id.btn_native_decode:
                result = "SHA256签名->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.SHA256);
                break;
            case R.id.btn_java_sign:
                result = "SHA384签名->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.SHA384);
                break;
            case R.id.btn_native_sign:
                result = "SHA512签名->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.SHA512);
                break;
            case R.id.btn_md5_sign:
                result = "MD5信息摘要->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.MD5).toUpperCase();
                break;
            case R.id.btn_AES_sign:
                result = "AES加密编码->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.AES);
                break;
            case R.id.btn_AESs_sign:
                result = "AES解密->" + SafetyUtil.getInstance().decode(this, ori, SafetyUtil.AES);
                break;
            case R.id.btn_AES_key:
                result = "AES加密编码->" + SafetyUtil.getInstance().encryptByAESEncrypt(this, ori, mKey);
                break;
            case R.id.btn_AESs_key:
                result = "AES解密->" + SafetyUtil.getInstance().decryptByAESEncrypt(this, ori, mKey);
                break;
            case R.id.btn_RSA_sign:
                result = "RSA公钥加密编码->" + SafetyUtil.getInstance().encode(this, ori, SafetyUtil.RSA_PUBKEY);
//                result ="RSA私钥加密编码->" + SafetyUtil.getInstance().encode(this,ori,SafetyUtil.RSA_PRIVATEKEY);
                break;
            case R.id.btn_RSAs_sign:
                result = "RSA私钥解密->" + SafetyUtil.getInstance().decode(this, ori, SafetyUtil.RSA_PRIVATEKEY);
//                result ="RSA公钥解密->" + SafetyUtil.getInstance().decode(this,ori,SafetyUtil.RSA_PUBKEY);
                break;
            case R.id.btn_RSAa_sign:
                result = "RSA公钥加密编码->" + SafetyUtil.getInstance().encryptRSA(this, ori, SafetyUtil.PUBLIC_KEY);
//                result ="RSA私钥加密编码->" +  SafetyUtil.getInstance().encodeByRSAPriKey(this,ori,SafetyUtil.PRIVATE_KEY);
                break;
            case R.id.btn_RSAd_sign:
                result = "RSA私钥解密->" + SafetyUtil.getInstance().decryptRSA(this, ori, SafetyUtil.PRIVATE_KEY);
//                result = "RSA公钥解密->" + SafetyUtil.getInstance().decodeByRSAPubKey(this,ori,SafetyUtil.PUBLIC_KEY);
                break;
