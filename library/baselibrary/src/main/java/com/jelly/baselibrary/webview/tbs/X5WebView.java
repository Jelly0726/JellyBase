...
import com.jelly.baselibrary.webview.DownPicUtil;
import com.jelly.baselibrary.webview.Utils;
...
public class X5WebView extends WebView {
	...
	private WebViewClient client = new WebViewClient() {
		...
		public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
			try {
				if (url.startsWith("http") || url.startsWith("https")) { //http和https协议开头的执行正常的流程
					...
					getContext().getApplicationContext().startActivity(in);
					return null;
				}
		...
		public void onReceivedHttpError(WebView view, WebResourceRequest request, WebResourceResponse error) {
			LogUtils.i("msg onReceivedHttpError", "request=" + new Gson().toJson(request) + " error=" + new Gson().toJson(error));
		...
		public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
			...
			//6.0以下执行
			LogUtils.i("onReceivedError: ------->errorCode" + errorCode + ",description=" + description);
		...
		public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
			...
			//6.0以上执行
			LogUtils.i("onReceivedError: ------->request="+new Gson().toJson(request)+",error"+ new Gson().toJson(error));
			LogUtils.i("onReceivedError: ------->isForMainFrame="+request.isForMainFrame());
			if(request.isForMainFrame()) {// 在这里加上个判断  或者： if(request.getUrl().toString() .equals(getUrl()))
				showErrorPage();//显示错误页面
			}
			if (tbsClientCallBack!=null){
...
//			loadUrl(uu);
	...
	private WebChromeClient chromeClient = new WebChromeClient() {
		...
		public void onShowCustomView(View view, IX5WebChromeClient.CustomViewCallback customViewCallback) {
			int id=getResources().getIdentifier("webfilechooser","id",getContext().getPackageName());
			if (id>0) {
				FrameLayout normalView = (FrameLayout) findViewById(id);
	...
	public X5WebView(Context arg0, AttributeSet arg1) {
		...
		setDownloadListener(new DownloadListener() {
			...
			public void onDownloadStart(String url, String userAgent, String contentDisposition, String mimeType, long contentLength) {
				...
				Utils.downloadBySystem(getContext(),url, contentDisposition, mimeType);
			}
		});
		// 长按点击事件 保存图片方法一
...
//			}
//		});
		MProgressUtil.getInstance().initialize(getContext().getApplicationContext());
	}
	private void initWebViewSettings() {
		...
		// * write权限
		webSetting.setGeolocationDatabasePath(getContext().getApplicationContext().getDir("geolocation", 0)
				.getPath());
		...
		// * 该方法应该只调用一次，多次调用自动忽略。
		webSetting.setAppCachePath(getContext().getApplicationContext().getDir("appcache", 0).getPath());
		webSetting.setDatabasePath(getContext().getApplicationContext().getDir("databases", 0).getPath());
	...
	public boolean tbs_dispatchTouchEvent(MotionEvent ev, View view) {
		...
		LogUtils.d("dispatchTouchEvent " + ev.getAction() + " " + r);
		return r;
	}
