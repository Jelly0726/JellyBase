package com.jelly.baselibrary.emil;
...
//    private static final String TO_ADD = "vicdaner@sina.com";//发到哪个邮件去
    ...
    public static void send(Context context,final File file, String toAdd){
        ...
        final MailInfo mailInfo = creatMail(context,"附件",toAdd);
    ...
    public static void send(Context context,final File file){
        final MailInfo mailInfo = creatMail(context,"附件",TO_ADD);
        final MailSender sms = new MailSender();
        new Thread(new Runnable() {
            @Override
            public void run() {
    ...
    public static void send(Context context,String toAdd){
        if (isEmail(toAdd)){
            final MailInfo mailInfo = creatMail(context,"测试文本",toAdd);
            final MailSender sms = new MailSender();
            new Thread(new Runnable() {
                @Override
                public void run() {
            ...
            final MailInfo mailInfo = creatMail(context,toAdd,TO_ADD);
            final MailSender sms = new MailSender();
            new Thread(new Runnable() {
                @Override
                public void run() {
    ...
    public static void send(Context context,String text,String toAdd){
        if (isEmail(toAdd)){
            new Throwable(new Exception("邮箱地址不正确"));
        }
        final MailInfo mailInfo = creatMail(context,text,toAdd);
        final MailSender sms = new MailSender();
        new Thread(new Runnable() {
            @Override
            public void run() {
    ...
    @NonNull
    private static MailInfo creatMail(Context context, String text, String toAdd) {
        ...
        String title="程序【"+context.getApplicationContext().getString(R.string.app_name)+"】崩溃啦";
