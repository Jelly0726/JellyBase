...
public class Config {
    ...
    public boolean isEnableWechat() {
        return AppPrefs.getBoolean(mContext, KEY_WECHAT_ENABLE);
    }

    /** 微信打开红包后的事件*/
    public int getWechatAfterOpenHongBaoEvent() {
        return AppPrefs.getInt(mContext, KEY_WECHAT_AFTER_OPEN_HONGBAO);
    }

    /** 微信抢到红包后的事件*/
    public int getWechatAfterGetHongBaoEvent() {
        return AppPrefs.getInt(mContext, KEY_WECHAT_AFTER_GET_HONGBAO, 1);
    }

    /** 微信打开红包后延时时间*/
    public int getWechatOpenDelayTime() {
        return AppPrefs.getInt(mContext, KEY_WECHAT_DELAY_TIME, 0);
    }

    /** 获取抢微信红包的模式*/
    public int getWechatMode() {
        return AppPrefs.getInt(mContext, KEY_WECHAT_MODE, 0);
    }

    /** 是否启动通知栏模式*/
    public boolean isEnableNotificationService() {
        return AppPrefs.getBoolean(mContext, KEY_NOTIFICATION_SERVICE_ENABLE, false);
    }

    public void setNotificationServiceEnable(boolean enable) {
        AppPrefs.putBoolean(mContext, KEY_NOTIFICATION_SERVICE_ENABLE, enable);
    }

    /** 是否开启声音*/
    public boolean isNotifySound() {
        return AppPrefs.getBoolean(mContext, KEY_NOTIFY_SOUND, true);
    }

    /** 是否开启震动*/
    public boolean isNotifyVibrate() {
        return AppPrefs.getBoolean(mContext, KEY_NOTIFY_VIBRATE, true);
    }

    /** 是否开启夜间免打扰模式*/
    public boolean isNotifyNight() {
        return AppPrefs.getBoolean(mContext, KEY_NOTIFY_NIGHT_ENABLE, false);
    }
